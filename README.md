...
6. 结合项目语义说明、父实验报告、当前实验命令、Git diff 比较信息、变更文件列表、聚焦后的代码/配置 diff、变更文件片段，以及可选的结果文件或目录摘要，调用 AI 生成结构化实验语义报告。
...
谱系图区域支持滚轮滚动，也可以在空白处按住拖拽平移；详情区中的 `record.md` 和 `Diff Preview` 会在内部滚动，避免长 diff 把页面整体撑得过长。
